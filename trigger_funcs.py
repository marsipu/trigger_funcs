...
def _get_load_cell_trigger(raw, trig_channel):
    ...
    rd1000_peaks, rd1000_props = find_peaks(
        abs(rolling_diff1000), height=std1000, distance=2500
    )
    ...
    rdstd100_peaks, rdstd100_props = find_peaks(
        abs(rolling_diffstd100), height=stdstd100, distance=100
    )
...
def get_load_cell_events(
    meeg, min_duration, shortest_event, adjust_timeline_by_msec, trig_channel
):
    raw = meeg.load_raw()
    ...
    (
        ...
        std2000,
        stdstd100,
    ) = _get_load_cell_trigger(raw, trig_channel)
    ...
    for pk in rd1000_peaks:
        ...
        if len(neg_diff_times) > 0:
            previous_id = int(
                events[np.nonzero(diff_times == np.max(neg_diff_times)), 2]
            )
        else:
            previous_id = 33
        ...
        if previous_id == 33 or previous_id == 1:
            try:
                ...
                trig_idx = 500 - (
                    np.nonzero(spoff[rd100lastidx:500] < np.std(spoff[:250]) * -3)[0][0]
                    + rd100lastidx
                )
...
def get_load_cell_events_regression(
    meeg,
    min_duration,
    shortest_event,
    ...
    for ev_idx, pk in enumerate(rd_peaks):
        ...
        best_idx = (
            scores.index(max(scores)) + reg_min
        )  # Compensate for reg_min because of range(reg_min, ...) above
        ...
        if idx == 0:
            ...
            if idx == len(events_meta) - 1:
                ...
                first_fill_time = first_time - (
                    events_meta[previous_idx]["last_time"] - eeg_raw.first_samp
                )
                last_fill_time = eeg_raw.n_times - (
                    events_meta[ev_idx]["last_time"] - eeg_raw.first_samp
                )
                reg_signal = np.concatenate(
                    [
                        reg_signal,
...
def plot_group_lc_coef(group, show_plots):
    ...
    ax.boxplot(ratings)
    ax.set_title(
        f"Load-Cell-Coef for {group.name} for {len(group.group_list)} subjects"
    )
...
def plot_lc_reg_raw(meeg, show_plots):
    ...
    reg_raw.plot(
        events,
        title=f"{meeg.name}_{meeg.p_preset}-Regression Fit for Load-Cell-Data",
        butterfly=False,
...
def plot_lc_latencies(group, show_plots):
    ...
    for meeg_name in group.group_list:
        ...
        diffs = [
            events_meta["Load"][idx]["first_time"]
            - events_meta["Touch"][idx]["first_time"]
            for idx in events_meta["Load"]
        ]
...
def _get_load_cell_trigger_model(
    meeg, min_duration, shortest_event, adjust_timeline_by_msec, trig_channel
):
    raw = meeg.load_raw()
    ...
    eeg_epochs = mne.Epochs(
        eeg_raw, events, event_id=event_id, tmin=0, tmax=7, baseline=None
    )
    ...
    for ep in epochs_data:
        ...
        lag_down, correl_down, max_lag_down, max_val_down = cross_correlation(
            ep_data, model_signal_down
        )
        lag_up, correl_up, max_lag_up, max_val_up = cross_correlation(
            ep_data, model_signal_up
        )
...
def plot_load_cell_ave(
    meeg, trig_plt_time, baseline_limit, show_plots, apply_savgol, trig_channel
):
    epochs_dict, times = _get_load_cell_epochs(
        meeg, trig_plt_time, baseline_limit, trig_channel, apply_savgol
    )
    fig, ax = plt.subplots(
        1, len(meeg.sel_trials), figsize=(5 * len(meeg.sel_trials), 8), sharey=True
    )
...
def plot_part_trigger(meeg, trig_channel, show_plots):
    ...
    plt.plot(
        [p for p in rd1000_peaks if tmin < p < tmax],
        [rolling_diff1000[p] for p in rd1000_peaks if tmin < p < tmax],
        "x",
        label="RollingDiff000-Peaks",
    )
    plt.plot(
        range(tmin, tmax), np.full(tmax - tmin, std1000), label="RollingDiff2000-Std"
    )
    plt.plot(
        range(tmin, tmax),
        np.full(tmax - tmin, stdstd100),
        label="RollingDiffStd100-Std",
    )
    plt.plot(
        [p for p in rd100_peaks if tmin < p < tmax],
        [rolling_diff100[p] for p in rd100_peaks if tmin < p < tmax],
        "x",
        label="RollingDiff100-Peaks",
    )
    plt.plot(
        [p for p in rdstd200_peaks if tmin < p < tmax],
        [rolling_diffstd200[p] for p in rdstd200_peaks if tmin < p < tmax],
        "x",
        label="RollingDiffStd200-Peaks",
    )
    plt.plot(
        [p for p in rdstd100_peaks if tmin < p < tmax],
        [rolling_diffstd100[p] for p in rdstd100_peaks if tmin < p < tmax],
        "x",
...
def plot_load_cell_trigger_raw(
    meeg,
    min_duration,
    shortest_event,
    adjust_timeline_by_msec,
...
def plot_ica_trigger(meeg, trig_channel, show_plots):
    ...
    raw_filtered = raw_filtered.copy().pick_types(
        meg=True, eeg=False, eog=False, stim=False, exclude=meeg.bad_channels
    )
    ica = meeg.load_ica()
    events = meeg.load_events()
    ...
    for trial in ltc_dict:
        for label in ltc_dict[trial]:
            ...
            for file in ltc_dict[trial][label]:
                plt.plot(
                    ltc_dict[trial][label][file][1],
                    ltc_dict[trial][label][file][0],
                    label=file,
                )
...
def get_wave_file(meeg, wav_input_type, ch_names, samplerate):
    ...
    for ch_name in ch_names:
        ...
        write(
            join(meeg.save_dir, f"{meeg.name}_{wav_input_type}_{ch_name}.wav"),
            samplerate,
            dp,
        )
